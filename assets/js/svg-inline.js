...
( function () {
	...
	const settings =
		typeof nanatoaddonsSvg !== 'undefined' ? nanatoaddonsSvg : {};
					...
					if ( node.nodeType === 1 ) {
						...
						if ( node.tagName === 'IMG' ) {
							if (
								forceInline &&
								node.src &&
								node.src.match( /\.svg$/i )
							) {
								if (
									! node.classList.contains( targetClass )
								) {
						...
						if ( node.querySelectorAll ) {
							...
							svgImages.forEach( function ( img ) {
								if (
									forceInline &&
									! img.classList.contains( targetClass )
								) {
									img.classList.add( targetClass );
								}
