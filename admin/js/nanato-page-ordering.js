...
(function( $ ) {
	...
	function updatePageOrderingCallback(response) {
		...
		if (changes.next) {
			...
			sortablePostTable.removeClass('nanato-updating').sortable('enable');
		}
	}
	...
	sortablePostTable.sortable({
		...
		create: function() {
			...
			$(document).keydown(function(e) {
				...
				if (key === 'Escape' || key === 'Esc' || key === 27) {
					sortablePostTable.sortable('option', 'preventUpdate', true);
		...
		update: function(e, ui) {
			...
			sortablePostTable.sortable('disable').addClass('nanato-updating');
